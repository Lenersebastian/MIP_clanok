...
function openEditProductPage(productName, productImage, productPrice) {
    window.location.href = `/edit_product/${productName}?image=${productImage}&price=${productPrice}`;
}
...
function generateProductItemsEdit() {
...
function showPreviousProducts() {
    ...
    generateProductItemsEdit(); // Regenerate product items for the current page
}
...
window.onload = function () {
    fetchProducts()
        .then(data => {
            productsData = data;
            generateProductItemsEdit(); // Generate product items with fetched data
